...
enum StatusItemIconRenderer {
    ...
    static func makeIcon(primaryUsed: Double?, secondaryUsed: Double?, isStale: Bool) -> NSImage {
        ...
        drawBar(y: 4.5, used: primaryUsed, alpha: alpha)
        drawBar(y: 11, used: secondaryUsed, alpha: alpha * 0.86)
    ...
    private static func drawBar(y: CGFloat, used: Double?, alpha: CGFloat) {
        ...
        let clamped = min(max(used / 100, 0), 1)
