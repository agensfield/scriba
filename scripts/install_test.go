...
func runInstaller(t *testing.T, goos, arch string, concurrency int) {
	...
	asset := fmt.Sprintf("scriba_9.8.7_%s_%s.tar.gz", assetOS, assetArch)
	payload := []byte("#!/bin/sh\nprintf '9.8.7\\n'\n")
...
`)
	env := append(os.Environ(),
		...
		"SCRIBA_VERSION=v9.8.7",
		"SCRIBA_INSTALL_DIR="+installDir,
		"TMPDIR="+tempDir,
	)
	...
	for range concurrency {
		...
		go func() {
			defer wait.Done()
			cmd := exec.Command("/bin/sh", "install.sh") // #nosec G204 -- fixed test command.
			cmd.Env = env
	...
	installed, err := exec.Command(filepath.Join(installDir, "scriba")).Output() // #nosec G204 -- test-owned path.
	if err != nil || string(installed) != "9.8.7\n" {
		t.Fatalf("installed=%q err=%v", installed, err)
	}
	for _, dir := range []string{tempDir, installDir} {
		entries, err := os.ReadDir(dir)
		if err != nil {
			t.Fatal(err)
		}
		for _, entry := range entries {
			if strings.HasPrefix(entry.Name(), "scriba-install.") || strings.HasPrefix(entry.Name(), ".scriba-install.") {
				t.Fatalf("temporary path survived: %s", filepath.Join(dir, entry.Name()))
			}
		}
	}
}
...
func writeInstallerArchive(t *testing.T, path string, payload []byte) {
	...
	if err := archive.WriteHeader(&tar.Header{Name: "scriba", Mode: 0o755, Size: int64(len(payload)), Typeflag: tar.TypeReg}); err != nil {
		t.Fatal(err)
	}
